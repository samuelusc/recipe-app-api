...
class PrivateRecipeApiTests(TestCase):
    ...
    def test_retrieve_recipes(self):
        ...
        create_recipe(user=self.user)
        # Exercise: Retrieve the list of recipes
        res = self.client.get(RECIPES_URL)
    ...
    def test_create_recipe_with_new_tags(self):
        ...
        self.assertEqual(res.status_code, status.HTTP_201_CREATED)
        recipes = Recipe.objects.filter(user=self.user)
        self.assertEqual(recipes.count(), 1)  # check if recipe is created
        recipe = recipes[0]  # otherwise recipe[0] will raise IndexError
    ...
    def test_create_recipe_with_existing_ingredients(self):
        ...
        self.assertEqual(recipes.count(), 1)
        recipe = recipes[0]
        # check if ingredients are added only 1 time
        self.assertEqual(recipe.ingredients.count(), 2)
    ...
    def test_update_recipe_assign_ingredient(self):
        ...
        # replace recipe ingredient from Salt to Pepper
        res = self.client.patch(url, payload, format='json')
